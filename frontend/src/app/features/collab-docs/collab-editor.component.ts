...
@Component({
  ...
  `],
  template: `
    <div class="flex h-screen flex-col bg-[#081726]">

      <!-- Topbar -->
      <div class="flex flex-wrap items-center gap-3 border-b border-blue-900/40 bg-blue-950 px-4 py-2 shadow-sm">
        <button (click)="goBack()" class="rounded-xl p-1.5 text-blue-300/70 transition hover:bg-[#0d1b2e] shrink-0">
          <mat-icon>arrow_back</mat-icon>
        </button>

        @if (docCtx()) {
          <div class="flex-1 min-w-0">
            <h1 class="truncate text-base font-bold text-white leading-tight">{{ docCtx()!.title }}</h1>
            <p class="text-xs text-blue-300/50">{{ connected() ? 'Conectado' : 'Desconectado' }} · {{ peers().length }} editando</p>
          </div>
          ...
          @if (isSpreadsheet()) {
            ...
            <div class="flex items-center gap-1 rounded-xl border border-blue-900/40 bg-[#081726] px-2 py-1 flex-wrap">
      ...
      <div class="relative flex-1 overflow-hidden">
        @if (loading()) {
          <div class="absolute inset-0 z-10 flex items-center justify-center bg-[#081726]">
        ...
        <div class="mx-auto max-w-4xl" [style.display]="isSpreadsheet() ? 'none' : 'block'">
          <div #editorRef class="tiptap-editor min-h-[calc(100vh-80px)] bg-blue-950 shadow-sm ring-1 ring-slate-100 mt-6 mx-4 mb-10 rounded-2xl overflow-hidden"></div>
        </div>
      </div>
    </div>
  `,
