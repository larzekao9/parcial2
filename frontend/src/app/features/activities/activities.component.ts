...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-[1400px] p-6">
      <div class="mb-5">
        <h2 class="m-0 text-2xl font-bold text-blue-50">Actividades</h2>
        <p class="mt-1.5 text-[13px] text-blue-300/70">Las tareas que tienes pendientes por rol, cargo o departamento.</p>
      </div>
      ...
      @if (isLoading()) {
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
          <mat-card class="min-h-[560px] rounded-[14px] !p-4">
            <div class="mb-3">
              <h3 class="m-0 text-base font-bold text-blue-50">Pendientes</h3>
            </div>

            @for (activity of activities(); track activity.id) {
              <button class="mb-2.5 w-full rounded-xl border border-blue-900/40 bg-blue-950 p-3 text-left"
                [class.border-blue-500]="selectedActivityId() === activity.id"
                ...
                <div class="mb-1.5 flex justify-between gap-2 text-xs text-blue-200">
                  <strong>{{ activity.currentNodoName }}</strong>
                  <span>{{ activity.code }}</span>
                </div>
                <div class="mb-1 text-sm font-semibold text-white">{{ activity.title }}</div>
                <div class="text-xs text-blue-300/70">{{ activity.workflowName }}</div>
              </button>
            } @empty {
              <div class="flex min-h-[220px] flex-col items-center justify-center gap-2.5 text-center text-blue-300/50">
          ...
          <mat-card class="min-h-[560px] rounded-[14px] !p-4">
            @if (isDetailLoading()) {
              ...
              <div class="mb-[18px]">
                <h3 class="m-0 text-[20px] font-semibold text-white">{{ selectedActivity()!.currentNodoName }}</h3>
                <p class="mt-1.5 text-[13px] text-blue-300/70">{{ selectedActivity()!.workflowName }} · {{ selectedActivity()!.code }}</p>
              </div>

              @if (selectedActivity()!.incomingData.length) {
                <section class="mb-[18px]">
                  <h4 class="mb-3 text-[15px] font-semibold text-blue-50">Datos compartidos</h4>
                  @for (block of selectedActivity()!.incomingData; track block.transitionId) {
                    <div class="mb-2.5 rounded-xl border border-blue-900/40 bg-[#081726] p-3">
                      <div class="mb-2.5 flex justify-between gap-2 text-xs text-blue-200">
                        <strong>{{ block.fromNodoName }}</strong>
                        <span>{{ block.transitionName || "Datos recibidos" }}</span>
                      </div>
                      @for (field of block.fields; track field.name) {
                        <div class="mb-2">
                          <label class="mb-1 block text-xs text-blue-300/70">{{ field.name }}</label>
                          <div class="rounded-[10px] border border-blue-900/40 bg-blue-950 p-2.5 text-[13px] text-white">
                            @if (field.type === 'GRID' && incomingGridColumns(field).length) {
                              <div class="overflow-x-auto">
                                <table class="min-w-full text-xs">
                                  <thead class="bg-[#081726] text-left uppercase tracking-wide text-blue-300/70">
                                    <tr>
                                  ...
                                  <tbody>
                                    @for (row of incomingGridRows(field); track rowIndex; let rowIndex = $index) {
                                      ...
                                      <tr>
                                        <td class="px-2 py-3 text-center text-blue-300/50" [attr.colspan]="incomingGridColumns(field).length">Sin filas</td>
                                      </tr>
                                    }
                                  </tbody>
                                </table>
                              ...
                              <div class="flex items-center gap-2">
                                <mat-icon class="!h-4 !w-4 !text-base text-blue-300/50">description</mat-icon>
                                <span class="flex-1 truncate text-sm text-blue-50">{{ uploadedFileName(field.value) }}</span>
                                @if (documentAccess().canRead) {
                                  <button type="button" class="flex items-center gap-1 rounded-lg border border-blue-900/40 px-2 py-1 text-xs text-blue-200 hover:bg-[#0d1b2e]" (click)="downloadFile(field.name, field.value)">
                                    <mat-icon class="!h-3.5 !w-3.5 !text-sm">download</mat-icon> Descargar
                                  </button>
                                }
                                @if (documentAccess().canEdit) {
                                  <button type="button" class="flex items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-xs font-semibold text-white hover:bg-indigo-700" (click)="openInEditor(field.name, asStoredFile(field.value))">
                              ...
                              <div class="flex flex-col gap-2">
                                @for (file of toUploadedFiles(field.value); track file.storedName) {
                                  <div class="flex items-center gap-2">
                                    <mat-icon class="!h-4 !w-4 !text-base text-blue-300/50">description</mat-icon>
                                    <span class="flex-1 truncate text-sm text-blue-50">{{ uploadedFileName(file) }}</span>
                                    @if (documentAccess().canRead) {
                                      <button type="button" class="flex items-center gap-1 rounded-lg border border-blue-900/40 px-2 py-1 text-xs text-blue-200 hover:bg-[#0d1b2e]" (click)="downloadFile(field.name, file)">
                                        <mat-icon class="!h-3.5 !w-3.5 !text-sm">download</mat-icon> Descargar
                                      </button>
                                    }
                                    @if (documentAccess().canEdit) {
                                      <button type="button" class="flex items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-xs font-semibold text-white hover:bg-indigo-700" (click)="openInEditor(field.name, asStoredFile(file))">
                                        <mat-icon class="!h-3.5 !w-3.5 !text-sm">edit</mat-icon> Editar
                                      </button>
                                    }
                                  </div>
              ...
              @if (tramiteFiles().length) {
                <section class="mb-[18px]">
                  <h4 class="mb-3 text-[15px] font-semibold text-blue-50">Archivos del trámite</h4>
                  <div class="flex flex-col gap-2">
                    @for (file of tramiteFiles(); track file.name) {
                      <div class="rounded-xl border border-blue-900/40 bg-[#081726] p-3">
                        <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-blue-300/70">{{ file.name }}</p>
                        @for (f of tramiteFileItems(file.value); track f.storedName) {
                          <div class="flex items-center gap-2 rounded-lg border border-blue-900/40 bg-blue-950 px-3 py-2">
                            <mat-icon class="!h-4 !w-4 !text-base text-blue-300/50">description</mat-icon>
                            <span class="flex-1 truncate text-sm text-blue-50">{{ f.fileName || f.storedName }}</span>
                            <button type="button" class="flex items-center gap-1 rounded-lg border border-blue-900/40 px-2 py-1 text-xs text-blue-200 hover:bg-[#0d1b2e]" (click)="downloadFile(file.name, f)">
                              <mat-icon class="!h-3.5 !w-3.5 !text-sm">download</mat-icon> Descargar
                            </button>
                            <button type="button" class="flex items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-xs font-semibold text-white hover:bg-indigo-700" (click)="openInEditor(file.name, f)">
                              <mat-icon class="!h-3.5 !w-3.5 !text-sm">edit</mat-icon> Editar colaborativamente
                            </button>
                          </div>
                        }
                        @if (documentAccess().canEdit) {
                          <label class="mt-2 flex cursor-pointer items-center gap-1 text-xs text-blue-400 hover:text-indigo-700">
              ...
              @if (formFields().length) {
                <section class="mb-[18px]">
                  <div class="mb-3 flex items-center justify-between gap-3">
                    <h4 class="text-[15px] font-semibold text-blue-50">{{ formTitle() }}</h4>
                  ...
                  @for (field of formFields(); track field.id) {
                    @if (field.type === "FILE") {
                      <div class="mb-4 flex flex-col gap-2">
                        <label class="text-[13px] font-medium text-blue-100">{{ field.name }}</label>
                        @if (canUploadForField(field)) {
                          <input class="text-[13px] text-blue-100" type="file" multiple (change)="uploadFiles(field, $event)" />
                        }
                        @if (fileItemsForField(field).length) {
                          <div class="flex flex-col gap-1 text-xs text-blue-400">
                            @for (file of fileItemsForField(field); track file.storedName) {
                              <button type="button" class="cursor-pointer border-none bg-transparent p-0 text-left font-inherit text-blue-400 underline" (click)="downloadFile(field.name, file)">{{ uploadedFileName(file) }}</button>
                            }
                          </div>
                        }
                      </div>
                      ...
                      <div class="mb-4">
                        ...
                        @if (gridColumns(field).length) {
                          <div class="overflow-x-auto rounded-xl border border-blue-900/40">
                            <table class="min-w-full text-sm">
                              ...
                              <tbody>
                                @for (row of gridRows(field); track rowIndex; let rowIndex = $index) {
                                    ...
                                    @for (column of gridColumns(field); track column.id) {
                                      <td class="px-3 py-2">
                                        @if (column.type === 'CHECKBOX') {
                                          ...
                                          <input
                                            class="w-full rounded-lg border border-blue-900/40 px-3 py-2 text-sm outline-none focus:border-indigo-400"
                                  ...
                                  <tr>
                                    <td class="px-3 py-4 text-center text-sm text-blue-300/50" [attr.colspan]="gridColumns(field).length + 1">Sin filas</td>
                                  </tr>
                                }
                              </tbody>
                            </table>
                          ...
                          <div class="rounded-xl border border-dashed border-blue-800/40 bg-[#081726] px-4 py-3 text-sm text-blue-300/70">
                            Esta grilla no tiene columnas configuradas.
                          </div>
                        }
                      </div>
                    } @else if (field.type === "CHECKBOX") {
                      <div class="mb-4 rounded-xl border border-blue-900/40 px-3 py-2">
              ...
              <div class="flex min-h-full flex-col items-center justify-center gap-2.5 text-center text-blue-300/50">
