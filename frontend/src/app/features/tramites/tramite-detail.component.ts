  ...
  template: `
    <div class="mx-auto flex w-full max-w-[1400px] flex-col gap-6 px-6 py-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-2xl font-bold text-white">{{ tramite()?.title }}</h2>
          <code class="rounded bg-[#0d1b2e] px-2 py-1 text-xs">{{ tramite()?.code }}</code>
        </div>
      ...
      @else if (tramite()) {
        <div class="grid gap-4 xl:grid-cols-2">
          <mat-card class="rounded-3xl p-5 shadow-sm">
            <h3 class="mb-3 text-base font-semibold text-white">Informacion</h3>
            <p class="mb-2 text-sm text-blue-200"><strong class="text-white">Descripcion:</strong> {{ tramite()!.description || 'Sin Descripcion' }}</p>
            <p class="text-sm text-blue-200"><strong class="text-white">Estado:</strong> <span class="ml-1 rounded-full px-3 py-1 text-xs font-semibold" [ngClass]="statusClass(tramite()!.status)">{{ tramite()!.status }}</span></p>
          </mat-card>

          <mat-card class="rounded-3xl p-5 shadow-sm">
            <h3 class="mb-3 text-base font-semibold text-white">Seguimiento</h3>
            <p class="mb-2 text-sm text-blue-200"><strong class="text-white">Etapa actual:</strong> {{ currentNodoName() || 'Sin etapa activa' }}</p>
            <p class="text-sm text-blue-200"><strong class="text-white">Workflow:</strong> {{ tramite()!.workflowId }}</p>
          </mat-card>


          <mat-card class="rounded-3xl p-5 shadow-sm xl:col-span-2">
            <h3 class="mb-3 text-base font-semibold text-white">Historial</h3>
              ...
              @for (h of tramite()!.history; track h.id) {
                <div class="relative flex gap-3 py-3">
                  ...
                  <div class="flex-1 pt-0.5">
                    ...
                    @if (h.nodoName) { <p class="mt-0.5 text-xs font-medium text-blue-100">{{ h.nodoName }}</p> }
                    @if (h.departmentName || h.jobRoleName) {
                      <p class="text-xs text-blue-300/70">{{ h.departmentName }}@if(h.departmentName && h.jobRoleName){<span class="mx-1 text-blue-400/40">·</span>}{{ h.jobRoleName }}</p>
                    }
                    @if (h.comment) { <p class="mt-0.5 text-xs italic text-blue-300/70">{{ h.comment }}</p> }
                    <p class="mt-1 text-xs text-blue-300/50">{{ h.changedAt | date:'dd/MM/yyyy HH:mm' }}</p>
                  </div>
                </div>
              }
              @if (tramite()!.status === 'COMPLETADO') {
                <div class="relative flex gap-3 py-3">
                  <div class="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-900/30 text-blue-400"><mat-icon class="!h-[18px] !w-[18px] !text-[18px]">flag</mat-icon></div>
                  <div class="flex-1 pt-0.5"><span class="text-sm font-semibold text-blue-400">FIN</span><p class="text-xs text-blue-300/70">Trámite completado</p></div>
                </div>
              }
            </div>
          </mat-card>
...
export class TramiteDetailComponent implements OnInit {
  ...
  statusClass(status: string) {
    return ({ PENDIENTE: 'bg-amber-100 text-amber-800', EN_PROGRESO: 'bg-blue-900/30 text-blue-300',
      COMPLETADO: 'bg-emerald-100 text-emerald-800', RECHAZADO: 'bg-rose-100 text-rose-800'
    } as Record<string, string>)[status] ?? 'bg-[#0d1b2e] text-blue-100';
  }
