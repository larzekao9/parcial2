...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-[1400px] p-6">
      ...
      <!-- Header -->
      <div class="mb-6">
        <h2 class="m-0 text-2xl font-bold text-blue-50">Auditoría documental</h2>
        <p class="mt-1 text-[13px] text-blue-300/70">
            ...
            <table class="min-w-full text-sm">
              ...
              <tbody>
                @for (item of entries(); track item.id) {
                  <tr class="border-t border-blue-900/30 hover:bg-[#081726] transition-colors">

                    <!-- Fecha -->
                    <td class="whitespace-nowrap px-4 py-3 text-blue-300/70">
                      {{ item.createdAt | date:'dd/MM/yyyy' }}<br>
                      <span class="text-xs text-blue-300/50">{{ item.createdAt | date:'HH:mm:ss' }}</span>
                    </td>
                    ...
                    <td class="max-w-[200px] px-4 py-3">
                      <div class="truncate font-medium text-blue-50" [title]="item.fileName || ''">
                        {{ (item.fileName || '-').replace('.docx.docx', '.docx') }}
                      </div>
                      @if (item.fieldName && item.fieldName !== 'collab') {
                        <div class="truncate text-xs text-blue-300/50">Campo: {{ item.fieldName }}</div>
                      }
                    </td>

                    <!-- Workflow -->
                    <td class="px-4 py-3 text-blue-300/70">{{ item.workflowName || '-' }}</td>


                    <!-- Usuario -->
                    <td class="px-4 py-3">
                      <div class="font-medium text-blue-50">{{ item.userName || '-' }}</div>
                    </td>

                    <!-- Departamento -->
                    <td class="px-4 py-3 text-blue-300/70">{{ item.departmentName || '-' }}</td>
                  ...
                  <tr>
                    <td colspan="8" class="px-4 py-12 text-center text-blue-300/50">
                      <mat-icon class="mb-2 !text-4xl text-blue-400/40">manage_search</mat-icon>
...
export class DocumentAuditComponent implements OnInit {
  ...
  actionCls(): string { return 'bg-[#0d1b2e] text-blue-100'; }
