...
@Component({
  ...
  `],
  template: `
    <div class="min-h-screen bg-[#081726]">

      <!-- Topbar -->
      <div class="sticky top-0 z-10 flex items-center gap-4 border-b border-blue-900/40 bg-blue-950 px-6 py-4 shadow-sm">
        <button
          class="flex items-center gap-1.5 rounded-xl border border-blue-900/40 bg-blue-950 px-3 py-2 text-sm font-medium text-blue-200 hover:bg-[#081726] transition"
          (click)="goBack()">
        ...
        @if (entry()) {
          <div class="flex flex-1 flex-wrap items-center gap-5 text-sm">
            <div>
              <span class="text-xs uppercase tracking-wide text-blue-300/50">Documento</span>
              <div class="font-semibold text-blue-50">{{ entry()!.fileName || 'Sin nombre' }}</div>
            </div>
            <div>
              <span class="text-xs uppercase tracking-wide text-blue-300/50">Editado por</span>
              <div class="font-medium text-blue-100">
                {{ entry()!.userName || '-' }}
                <span class="ml-1 text-xs text-blue-300/50">{{ entry()!.userEmail }}</span>
              </div>
            </div>
            <div>
              <span class="text-xs uppercase tracking-wide text-blue-300/50">Fecha</span>
              <div class="font-medium text-blue-100">{{ entry()!.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}</div>
            </div>
            <div>
              <span class="text-xs uppercase tracking-wide text-blue-300/50">Workflow</span>
              <div class="font-medium text-blue-100">{{ entry()!.workflowName || '-' }}</div>
            </div>
          </div>
      ...
      <div class="mx-auto max-w-[1600px] p-6">
        ...
        @if (!entry()) {
          <div class="flex flex-col items-center justify-center py-24 text-blue-300/50">
            <mat-icon class="mb-3 !text-5xl text-blue-400/40">find_in_page</mat-icon>
            <p class="text-lg font-medium">No hay datos de comparación.</p>
            <button class="mt-4 rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"
          ...
          @if (isGrid()) {
            <div class="overflow-hidden rounded-[18px] border border-blue-900/40 bg-blue-950 shadow-sm">
              <!-- Cabeceras de panel -->
              <div class="grid grid-cols-2 border-b border-blue-900/40">
                <div class="flex items-center gap-2 border-r border-blue-900/40 bg-rose-50 px-5 py-3">
              ...
              <!-- Leyenda -->
              <div class="flex items-center gap-6 border-b border-blue-900/30 bg-[#081726] px-5 py-2 text-xs text-blue-300/70">
              ...
              <div class="overflow-auto max-h-[calc(100vh-220px)]">
                <div class="flex min-w-fit">
                  ...
                  <!-- Grilla Antes -->
                  <div class="flex-1 min-w-0 border-r border-blue-900/40">
            ...
            <div class="overflow-hidden rounded-[18px] border border-blue-900/40 bg-blue-950 shadow-sm">
              ...
              <div class="flex items-center gap-6 border-b border-blue-900/30 bg-[#081726] px-5 py-2 text-xs text-blue-300/70">
                ...
                <span class="flex items-center gap-1.5">
                  <span class="inline-block h-3 w-6 rounded bg-[#0d1b2e]"></span> Sin cambios
                </span>
              </div>

              @if (!sideRows().length) {
                <div class="px-6 py-12 text-center text-blue-300/50">Sin diferencias detectadas.</div>
              } @else {
                <div class="overflow-auto max-h-[calc(100vh-220px)] font-mono text-sm">
                  @for (row of sideRows(); track $index) {
                    <div class="grid grid-cols-2 border-t border-slate-50 leading-6">
                      ...
                      <div class="flex items-stretch border-r border-blue-900/30"
                           [class.bg-rose-50]="row.left.type === 'removed'"
                           [class.bg-[#081726]]="row.left.type === 'unchanged'"
                           [class.bg-blue-950]="row.left.type === 'empty'">
                        <div class="w-10 shrink-0 select-none border-r px-1 py-1 text-right text-xs text-blue-400/40"
                             [class.border-rose-100]="row.left.type === 'removed'"
                             [class.border-blue-900]="row.left.type !== 'removed'">
                          {{ row.left.lineNo ?? '' }}
                        </div>
                        ...
                        <div class="flex-1 py-1 pr-4 whitespace-pre-wrap break-words"
                             ...
                             [class.text-blue-400]="row.left.type === 'unchanged'"
                      ...
                      <div class="flex items-stretch"
                           ...
                           [class.bg-[#081726]]="row.right.type === 'unchanged'"
                           [class.bg-blue-950]="row.right.type === 'empty'">
                        <div class="w-10 shrink-0 select-none border-r px-1 py-1 text-right text-xs text-blue-400/40"
                             [class.border-emerald-100]="row.right.type === 'added'"
                             [class.border-blue-900]="row.right.type !== 'added'">
                          {{ row.right.lineNo ?? '' }}
                        </div>
                        <div class="flex w-6 shrink-0 items-center justify-center text-sm font-bold select-none"
                        ...
                        <div class="flex-1 py-1 pr-4 whitespace-pre-wrap break-words"
                             ...
                             [class.text-blue-400]="row.right.type === 'unchanged'"
