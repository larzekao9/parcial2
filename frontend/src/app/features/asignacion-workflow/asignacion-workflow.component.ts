...
@Component({
  selector: 'app-asignacion-workflow',
  standalone: true,
  ...
  template: `
    <div class="mx-auto max-w-[1300px] p-6 space-y-6">
      ...
      <!-- Header -->
      <div>
        <h2 class="m-0 text-2xl font-bold text-blue-50">Usuario Pide</h2>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="space-y-4">
          ...
          <div class="rounded-[18px] border border-blue-900/40 bg-blue-950 p-5 shadow-sm">
            <h3 class="mb-3 text-sm font-semibold text-blue-100">
              <mat-icon class="mr-1 !text-[16px] align-middle text-blue-400">mic</mat-icon>
            ...
            <div class="flex gap-3">
              ...
              <textarea
                class="flex-1 resize-none rounded-xl border border-blue-900/40 bg-[#081726] p-3 text-sm placeholder:text-blue-300/50 focus:border-indigo-400 focus:bg-blue-950 focus:outline-none"
                rows="4"
                [(ngModel)]="userText"
          ...
          <div class="rounded-[18px] border border-blue-900/40 bg-blue-950 p-5 shadow-sm">
            ...
            <p class="mb-3 text-xs text-blue-300/50">
            ...
            <!-- Drop zone -->
            <div
              class="drop-zone flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-blue-800/40 bg-[#081726] px-4 py-8 text-center transition hover:border-indigo-400 hover:bg-indigo-50"
              (click)="fileInput.click()"
              (dragover)="onDragOver($event)"
              (dragleave)="onDragLeave($event)"
              (drop)="onDrop($event)">
              <mat-icon class="mb-2 !text-4xl text-blue-400/40">cloud_upload</mat-icon>
              <p class="text-sm font-medium text-blue-300/70">
                Arrastrá archivos aquí o hacé clic para seleccionar
              </p>
              <p class="mt-1 text-xs text-blue-300/50">.docx · .pdf · .txt</p>
            </div>
            ...
            @if (selectedFiles.length) {
              <div class="mt-3 space-y-2">
                @for (f of selectedFiles; track f.name; let i = $index) {
                  <div class="flex items-center justify-between rounded-xl border border-blue-900/40 bg-blue-950 px-3 py-2">
                    <div class="flex items-center gap-2 min-w-0">
                      <mat-icon class="shrink-0 !text-[20px] text-blue-400">
                        {{ fileIcon() }}
                      </mat-icon>
                      <div class="min-w-0">
                        <p class="truncate text-xs font-medium text-blue-100">{{ f.name }}</p>
                        <p class="text-xs text-blue-300/50">{{ (f.size / 1024).toFixed(0) }} KB</p>
                      </div>
                    </div>
                    <button (click)="removeFile(i)"
                      class="ml-2 shrink-0 rounded-full p-1 text-blue-300/50 hover:bg-rose-50 hover:text-rose-500 transition">
          ...
          <!-- Botón analizar -->
          <button
            class="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-50 transition"
        ...
        <div class="space-y-4">
          ...
          @if (analyzedDocs().length) {
            <div class="rounded-[18px] border border-blue-900/40 bg-blue-950 p-5 shadow-sm">
              <h3 class="mb-3 text-sm font-semibold text-blue-100">
                <mat-icon class="mr-1 !text-[16px] align-middle text-blue-400">auto_awesome</mat-icon>
                Documentos leídos por TensorFlow
              </h3>
              <div class="space-y-2">
                @for (doc of analyzedDocs(); track doc.filename) {
                  <div class="flex items-start gap-3 rounded-xl border border-blue-900/30 bg-[#081726] p-3">
                    <mat-icon class="shrink-0 !text-[22px] text-blue-400 mt-0.5">
                      {{ docTypeIcon() }}
                    </mat-icon>
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center justify-between gap-2">
                        <p class="truncate text-xs font-semibold text-blue-100">{{ doc.filename }}</p>
                      ...
                      @if (doc.preview) {
                        <p class="mt-1 line-clamp-2 text-xs text-blue-300/50">{{ doc.preview }}</p>
                      }
                    </div>
                  </div>
                }
          ...
          @if (!matches().length && !loading() && !analyzedDocs().length) {
            <div class="flex flex-col items-center justify-center rounded-[18px] border border-dashed border-blue-900/40 bg-[#081726] p-10 text-center text-blue-300/50">
              <mat-icon class="mb-3 !text-5xl text-blue-400/40">account_tree</mat-icon>
          ...
          @if (loading()) {
            <div class="flex flex-col items-center justify-center rounded-[18px] border border-blue-900/40 bg-blue-950 p-10">
              <mat-spinner [diameter]="40" />
              <p class="mt-4 text-sm text-blue-300/70">Leyendo documentos y analizando con TensorFlow…</p>
            </div>
          }
          ...
          @if (matches().length && !loading()) {
            <div class="space-y-3">
              <p class="text-xs font-semibold uppercase tracking-wide text-blue-300/50">
                Workflows recomendados
              </p>

              @for (m of matches(); track m.workflowId; let i = $index) {
                <div class="rounded-[18px] border bg-blue-950 p-5 shadow-sm"
                     [class.border-indigo-300]="i === 0"
                     [class.ring-1]="i === 0"
                     [class.ring-blue-500]="i === 0"
                     [class.border-blue-900]="i !== 0">
                  ...
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div>
                        <p class="font-semibold text-blue-50">{{ m.workflowName }}</p>
                        @if (m.workflowDescription) {
                          <p class="mt-0.5 text-xs text-blue-300/70">{{ m.workflowDescription }}</p>
                        }
                        <p class="mt-0.5 text-xs text-blue-300/50">Similitud: {{ m.cosSim }}%</p>
                      </div>
                    </div>
                  ...
                  <!-- Barra de progreso -->
                  <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-[#0d1b2e]">
                  ...
                  @if (m.requiredDocs.length) {
                    <div class="mt-4">
                      <p class="mb-2 text-xs font-semibold text-blue-300/70">
                  ...
                  @if (m.optionalDocs.length) {
                    <div class="mt-2">
                      <p class="mb-2 text-xs font-semibold text-blue-300/50">Campos opcionales:</p>
                      <div class="flex flex-wrap gap-1.5">
                        @for (doc of m.optionalDocs; track doc) {
                          <span class="flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs"
                                [ngClass]="m.presentOptional.includes(doc)
                                  ...
                                  : 'border-blue-900/40 bg-[#081726] text-blue-300/50'">
                  ...
                  @if (i === 0) {
                    <button (click)="iniciarTramite(m)"
                      class="mt-4 w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition">
                      <mat-icon class="!text-[18px]">play_arrow</mat-icon>
                      Iniciar trámite con este workflow
                    </button>
                  } @else {
                    <button (click)="iniciarTramite(m)"
                      class="mt-3 w-full flex items-center justify-center gap-2 rounded-xl border border-blue-900/40 py-2 text-sm text-blue-200 hover:bg-[#081726] transition">
                      Usar este workflow
                    </button>
                  }
                </div>
...
export class AsignacionWorkflowComponent implements OnDestroy {
  ...
  scoreColor(s: number)    { return s >= 70 ? 'text-emerald-600' : s >= 40 ? 'text-amber-500' : 'text-blue-300/50'; }
  ...
  confidenceCls(c: string) {
    ...
    return 'bg-[#0d1b2e] text-blue-300/70';
  }
