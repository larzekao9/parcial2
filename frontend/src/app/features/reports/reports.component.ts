  ...
  template: `
    <div class="mx-auto max-w-7xl p-6">
      <div class="mb-6 flex items-center justify-between gap-4">
        <div>
          <h2 class="text-3xl font-bold text-white">Reportes</h2>
          <p class="text-sm text-blue-300/70">Resumen general del sistema.</p>
        </div>
        <div class="rounded-full border px-3 py-1 text-sm" [class.border-emerald-200]="realtimeConnected()" [class.bg-emerald-50]="realtimeConnected()" [class.text-emerald-700]="realtimeConnected()" [class.border-blue-900]="!realtimeConnected()" [class.bg-[#0d1b2e]]="!realtimeConnected()" [class.text-blue-200]="!realtimeConnected()">
      ...
      @if (loading()) {
        ...
        <div class="mb-6">
          <div class="rounded-2xl border border-blue-900/40 bg-blue-950 p-5"><div class="text-sm text-blue-300/70">Total tramites</div>
          <div class="mt-2 text-4xl font-bold text-white">{{ stats()?.totalTramites ?? 0 }}</div></div>
        </div>

        <div class="grid gap-6 xl:grid-cols-2">
          <section class="rounded-2xl border border-blue-900/40 bg-blue-950 p-5">
            <h3 class="mb-4 text-lg font-semibold text-white">Tramites por estado</h3>
            <div class="space-y-3">
              @for (item of statusEntries(); track item.key) {
                <div class="flex items-center justify-between rounded-xl bg-[#081726] px-4 py-3">
                  <span class="font-medium text-blue-100">{{ item.key }}</span>
                  <span class="font-bold text-white">{{ item.value }}</span>
                </div>
              } @empty {
                <div class="text-sm text-blue-300/50">Sin datos</div>
              }
            </div>
          </section>

          <section class="rounded-2xl border border-blue-900/40 bg-blue-950 p-5">
            <h3 class="mb-4 text-lg font-semibold text-white">Departamentos con mayor flujo</h3>
            <div class="space-y-3">
              @for (item of topDepartments(); track item.departmentName) {
                <div class="flex items-center justify-between rounded-xl bg-[#081726] px-4 py-3">
                  <span class="font-medium text-blue-100">{{ item.departmentName }}</span>
          ...
          <section class="rounded-2xl border border-blue-900/40 bg-blue-950 p-5 xl:col-span-2">
            <h3 class="mb-4 text-lg font-semibold text-white">Antes del Tiempo Estimado</h3>
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="border-b border-blue-900/40 text-left text-blue-300/70">
                ...
                <tbody>
                  @for (item of tempranoRoles(); track item.departmentName + item.jobRoleName) {
                    <tr class="border-b border-blue-900/30"><td class="py-2">{{ item.departmentName }}</td>
                    ...
                    <tr><td colspan="5" class="py-4 text-center text-blue-300/50">Sin datos</td></tr>
                  }
                </tbody>
              </table>
            </div>
          ...
          <section class="rounded-2xl border border-blue-900/40 bg-blue-950 p-5 xl:col-span-2">
            <h3 class="mb-4 text-lg font-semibold text-white">Despues del Tiempo Estimado</h3>
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="border-b border-blue-900/40 text-left text-blue-300/70">
                  <tr><th class="py-2">Departamento</th><th class="py-2">Rol</th><th class="py-2">Cantidad</th>\
