  ...
  template: `
    <div class="mx-auto flex w-full max-w-[1400px] flex-col gap-6 px-6 py-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-3xl font-bold text-white">Gestionar roles</h2>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
              <thead class="bg-[#081726] text-left text-xs font-semibold uppercase tracking-wide text-blue-300/70">
                <tr><th class="px-4 py-3">Rol</th><th class="px-4 py-3">Departamento</th><th class="px-4 py-3">Acciones</th></tr>
              </thead>
              <tbody>
                @for (jobRole of jobRoles(); track jobRole.id) {
                  ...
                  <tr><td colspan="4" class="px-4 py-10 text-center text-blue-300/50">No hay roles registrados</td></tr>
                }
              </tbody>
            </table>
          </div>
      ...
      @if (showForm()) {
        <div class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70 px-4" (click)="showForm.set(false)">
          <mat-card class="w-full max-w-lg rounded-3xl p-6 shadow-2xl" (click)="$event.stopPropagation()">
            <h3 class="mb-4 text-xl font-semibold text-white">{{ editId() ? 'Editar' : 'Nuevo' }} rol</h3>
