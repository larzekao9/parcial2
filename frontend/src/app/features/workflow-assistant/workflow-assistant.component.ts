...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-[1500px] p-6">
      <div class="mb-5">
        <h2 class="m-0 text-2xl font-bold text-blue-50">Asistente de solicitudes</h2>
        <p class="mt-1.5 text-[13px] text-blue-300/70">Describe el problema por texto o voz, adjunta documentos y la IA te propone el workflow mas especifico.</p>
      </div>

      <div class="grid gap-4 xl:grid-cols-[460px_minmax(0,1fr)]">
        <mat-card class="rounded-[18px] !p-5">
          <div class="mb-4 flex items-center justify-between gap-3">
            <h3 class="m-0 text-lg font-semibold text-white">Solicitud</h3>
          ...
          <div class="mb-4">
            <label class="mb-2 block text-sm font-medium text-blue-100">Documentacion de apoyo</label>
            <input type="file" multiple (change)="onSupportFilesSelected($event)">
            @if (supportFiles().length) {
              <div class="mt-3 flex flex-col gap-2 text-sm text-blue-200">
                @for (file of supportFiles(); track file.name + '-' + file.size) {
                  <div class="flex items-center justify-between gap-3 rounded-xl border border-blue-900/40 px-3 py-2">
                    <div class="min-w-0">
                      <div class="truncate font-medium text-blue-50">{{ file.name }}</div>
                      <div class="text-xs text-blue-300/70">{{ humanSize(file.size) }}</div>
                    </div>
        ...
        <mat-card class="rounded-[18px] !p-5">
          @if (classifying()) {
            ...
            <div class="flex min-h-[320px] flex-col items-center justify-center gap-3 text-center text-blue-300/50">
            ...
            <div class="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div>
                <div class="text-xs font-semibold uppercase tracking-wide text-blue-400">Workflow sugerido</div>
                <h3 class="m-0 text-xl font-semibold text-white">{{ suggestion()!.workflowName || 'Sin coincidencia clara' }}</h3>
                <p class="mt-1 text-sm text-blue-300/70">{{ suggestion()!.detectedIntent || '' }}</p>
              </div>
            ...
            @if (suggestion()!.reasoning) {
              <div class="mb-4 rounded-2xl border border-blue-900/40 bg-[#081726] p-3 text-sm text-blue-100">
            ...
            @if (suggestion()!.suggestedQuestions?.length) {
              <div class="mb-4 rounded-2xl border border-sky-200 bg-sky-50 p-3">
                <div class="mb-2 text-sm font-semibold text-white">Preguntas sugeridas</div>
                <ul class="m-0 list-disc pl-5 text-sm text-blue-100">
            ...
            @if (selectedWorkflow() && entryNodo()) {
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <div class="text-sm font-semibold text-white">{{ entryNodo()!.name }}</div>
                  <div class="text-xs text-blue-300/70">{{ entryNodo()!.formDefinition?.title || 'Formulario inicial' }}</div>
                </div>
              </div>
                ...
                <div class="mb-4 rounded-2xl border border-rose-200 bg-rose-50 p-3">
                  ...
                  <div class="flex flex-wrap gap-2">
                    @for (field of missingRequiredFields(); track field) {
                      <span class="rounded-full bg-blue-950 px-3 py-1 text-xs font-semibold text-rose-700">{{ field }}</span>
                    }
                  </div>
                </div>
              }
              ...
              @for (field of entryFormFields(); track field.id) {
                @if (field.type === 'FILE') {
                  <div class="mb-4 flex flex-col gap-2">
                    <label class="text-sm font-medium text-blue-100">{{ field.name }}</label>
                    ...
                    @if (workflowFileItems(field).length) {
                      <div class="flex flex-col gap-1">
                        @for (file of workflowFileItems(field); track file.storedName) {
                          <button type="button" class="bg-transparent p-0 text-left text-xs text-blue-400 underline" (click)="downloadFile(file)">{{ fileLabel(file) }}</button>
                        }
                      </div>
                    }
                  </div>
                                ...
                                @for (column of gridColumns(field); track column.id) {
                                  <td class="px-3 py-2">
                                    @if (column.type === 'CHECKBOX') {
                                      ...
                                      <input class="w-full rounded-lg border border-blue-900/40 px-3 py-2 text-sm outline-none focus:border-indigo-400"
